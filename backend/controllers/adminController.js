    ...
    for (const report of reports) {
      if (!listingsMap.has(report.listingId)) {
        const listingDoc = await db
          ...
          .get();
        if (listingDoc.exists)
          listingsMap.set(report.listingId, {
            id: report.listingId,
            ...listingDoc.data(),
          });
      }
      if (!usersMap.has(report.reportedUserId)) {
        const userDoc = await db
          ...
          .get();
        if (userDoc.exists)
          usersMap.set(report.reportedUserId, {
...
exports.dismissReport = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error dismissing report", error: error.message });
  }
};
...
exports.removeReportedListing = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Listing and related reports removed successfully" });
  } catch (error) {
    console.error("Error removing reported listing:", error);
    res
      .status(500)
      .json({ message: "Error removing listing", error: error.message });
  }
};
...
exports.getReportCounts = async (req, res) => {
  try {
    ...
    reportsSnapshot.forEach((doc) => {
      ...
      listingReportCounts[listingId] =
        (listingReportCounts[listingId] || 0) + 1;
      userReportCounts[reportedUserId] =
        (userReportCounts[reportedUserId] || 0) + 1;
    });
    ...
    res
      .status(500)
      .json({ message: "Error fetching report counts", error: error.message });
  }
};
// View reported listings
exports.viewReportedListings = async (req, res) => {
  try {
    ...
    for (const report of reports) {
      ...
      listingReportCounts[report.listingId] =
        (listingReportCounts[report.listingId] || 0) + 1;
      userReportCounts[report.reportedUserId] =
        (userReportCounts[report.reportedUserId] || 0) + 1;
    }
