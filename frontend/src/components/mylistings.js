...
import { Card, Button, Row, Col, Container, Badge, Dropdown, Spinner, Alert } from "react-bootstrap"
...
const MyListings = () => {
  ...
  const fetchMyListings = async () => {
    try {
      ...
      if (!currentUser || !currentUser.uid) {
        setError("User not authenticated. Please log in to view your listings.")
        setLoading(false)
        return
      }

      // Use the correct API endpoint with the user ID
      const response = await axios.get(`${process.env.REACT_APP_BACKEND}/api/my-listings/${currentUser.uid}`)
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this listing? This action cannot be undone.")) {
      try {
        ...
        const response = await axios.delete(`${process.env.REACT_APP_BACKEND}/api/listings/${id}`, {
          headers: {
            Authorization: `Bearer ${await currentUser.getIdToken()}` // Include auth token for verification
          }
        });
  ...
  const handleEdit = (item) => {
    // Your existing or updated handleEdit function
    window.location.href = `/editListing/${item.id}`;
      ...
      {listings.length === 0 && (
        <Card className="text-center p-5 border-0 shadow-sm">
          <Card.Body>
            ...
            <Button variant="outline-primary" className="mt-3" onClick={handleAddNew}>
              ...
              <Row className="g-0 align-items-center">
                ...
                <Col xs={4} md={3} className="p-3">
                  <div className="position-relative">
                    <Card.Img
                      src={(item.picUrls && item.picUrls.length > 0) ? item.picUrls[0] : "https://via.placeholder.com/100"}
                      alt={item.title}
                    ...
                    <div className="d-flex justify-content-between align-items-start mb-2">
                      ...
                      <Dropdown align="end">
                        ...
                        <Dropdown.Menu>
                          ...
                          {item.status === "active" ? (
                            <Dropdown.Item onClick={() => handleStatusUpdate(item.id, "inactive")}>
                              <i className="bi bi-check-circle me-2"></i> Mark as Sold
                            </Dropdown.Item>
                          ) : (
                            <Dropdown.Item onClick={() => handleStatusUpdate(item.id, "active")}>
                              <i className="bi bi-arrow-counterclockwise me-2"></i> Mark as Unsold
                            </Dropdown.Item>
                          )}
                        ...
                        {item.status === "active" ? (
                          ...
                          >
                            {statusLoading === item.id ? (
                              ...
                              <>
                                <i className="bi bi-check-circle me-1"></i> Mark as Sold
                              </>
                            )}
                          </Button>
                        ) : (
                          ...
                          >
                            {statusLoading === item.id ? (
                              ...
                              <>
                                <i className="bi bi-arrow-counterclockwise me-1"></i> Mark as Unsold
                              </>
                            )}
                          </Button>
                        )}
